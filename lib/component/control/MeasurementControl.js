...
class MeasurementControl extends HTMLElement {
  ...
  /**
   * @private
   * @type { import('../../service/MapService') }
   */
  _mapService;

  /**
   * @private
   * @type { import('../../service/ControlService') }
   */
  _controlService;
  ...
  constructor() {
    ...
    this._mapService = new MapService(this._map);
    this._controlService = new ControlService(this._map);
  }
  ...
  addEventListner() {
    ...
    $line.addEventListener('click', (e) =>
      this.measurementHandler(e, 'distance')
    );
  ...
  measurementHandler(e, tool) {
    ...
    switch (tool) {
      case 'area':
        ...
        if (this._controlService.currentControl !== CONTROL.areaMeasurement) {
          this.addInteraction('Polygon');
          this._controlService.currentControl = CONTROL.areaMeasurement;
        } else {
          this._controlService.currentControl = CONTROL.none;
        }
        break;
      case 'distance':
        this._map.removeInteraction(this.draw);
        if (
          ...
          this._controlService.currentControl = CONTROL.distanceMeasurement;
        } else {
          this._controlService.currentControl = CONTROL.none;
        }
        break;
   ...
   */
  addInteraction(type) {
    const measurementLayer = this._mapService.getLayerById('measurement-layer');
    ...
    this.draw.set(
      'id',
      type === 'Polygon' ? CONTROL.areaMeasurement : CONTROL.distanceMeasurement
    );
    this._controlService.addControl(this.draw);
